...
import { init, configFile, appDir, dateFormats } from "./utils.js";
import { track, setDefaultThing, argvToThingOptions } from "./thing.js";
import { show, availableOutputs } from "./output.js";
    ...
    .command(
        ...
        (yargs, helpOrVersionSet) => {
            ...
            yargs
                ...
                .option("timestamp", {
                    type: "string",
                    default: format(Date.now(), dateFormats[0]),
        ...
        (yargs, helpOrVersionSet) => {
            ...
            yargs
                ...
                .options("daysSince", {
                    type: "number",
                    alias: "d",
                    description: "number of days to show until now",
                })
                .option("format", {
                    alias: "f",
                    default: "calendar",
                    description: "what format to output",
                    choices: availableOutputs(),
                });

            return yargs;
        },
        async (argv) => {
            await show(argv.thing, argv);
        },
    )
    .command(
        "set <thing>",
